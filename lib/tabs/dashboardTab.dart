...
import 'package:bloomzon_estate/classes/provider.dart';
import 'package:bloomzon_estate/services/approvedServices.dart';
import 'package:bloomzon_estate/services/serviceChecker.dart';
import 'package:bloomzon_estate/services/propertyChecker.dart';
import 'package:bloomzon_estate/views/messages.dart';
import 'package:bloomzon_estate/views/notifications.dart';
import 'package:bloomzon_estate/views/profile.dart';
...
//  }
  ...
  Future getData() async {
    var response = await http.get(
        Uri.encodeFull("https://bloomzon.com/estate/api/fetch_featured_api.php"),
...
//  }
  ...
  getCountries() async {
    var response = await http.get(
        Uri.encodeFull("https://raw.githubusercontent.com/dr5hn/countries-states-cities-database/master/countries.json"),
        headers: {"Accept": "application/json"});
                      ...
                      : filteredProperties.length < 1 && _current == 0
                          ...
                          : ListView.builder(
                              ...
                              itemBuilder: (BuildContext context, int index) {
                                // { 190000, 08137201070, Ose Ose, US property UK, Rent, null }
                                // if(index == 1 ){
                                  print(filteredProperties[1]);
                                final item = filteredProperties[index];
                                return   PropertyList(
                                  amount: filteredProperties[index].amount ?? 0,
                                  imagePath: filteredProperties[index].img1 ?? "https://bloomzon.com/estate/gallery/10-quick-tips-about-real-estate-86018.jpg",
                                  location: filteredProperties[index].address ?? 'NOt stated',
                                  propId: filteredProperties[index].prop_id ?? 0,
                                  region: filteredProperties[index].region ?? 'Null',
                                  saleOrRent: filteredProperties[index].status ?? 'null',
                                  title: filteredProperties[index].title ?? 'null',
                                  phone: filteredProperties[index].phone ?? 'n',
                                  state: filteredProperties[index].state ?? 'n',
                                  name: filteredProperties[index].name ?? 'n',
                                  email: filteredProperties[index].user_email ?? 'n',
                                  isFav: filteredProperties[index].isFav ?? 'n',
                                  item: item ?? {},
                                  goto: EachProperty(
                                    item: item,
                                  ),
